...
#' 
#' @fnname native \code{grid} function name.  
#' 
#' @keywords internal
#' @rdname grid 
...
#{
#	grid:::vpFromPushedvp(grid.Call(grid:::L_currentViewport))	
#}
#
#
##' @rdname grid
...
#' 
#' \code{tryViewport}It uses \code{\link[grid]{gird.ls}} and not 
...
tryViewport <- function(name, verbose=FALSE){
	...
	l <- grid.ls(viewports=TRUE, grob=FALSE, print=FALSE)
	if( name %in% l$name ){
		downViewport(name)
	}	
}
