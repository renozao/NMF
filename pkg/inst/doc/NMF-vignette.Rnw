...
% \VignetteDepends{NMF,Biobase,bigmemory,xtable}
...
% Environment for highlighting sections
\newenvironment{highlight}{\definecolor{shadecolor}{rgb}{0.92,0.92,1}%
\begin{shaded}\sffamily}%
{\end{shaded}}
...
\title{Using the Package NMF\\\small Version \Sexpr{utils::packageVersion('NMF')}}
\author{Renaud Gaujoux, \email{renaud@cbio.uct.ac.za}}
% \\Address Computational Biology - University of Cape Town, South Africa,

\maketitle

This vignette presents the package NMF \cite{Gaujoux2010}, which implements a framework for Nonnegative Matrix Factorization (NMF) algorithms in R \cite{R}. 
The objective is to provide an implementation of some standard algorithms, while allowing the user to easily implement new methods that integrate into the package's framework.

\bigskip
To cite the package \nmfpack in publications please use:
...
standard calls to \code{library}.
\columnbreak
...
\item $R$ is an optional regularization function, defined to enforce desirable properties on matrices $W$ and $H$, such as smoothness or sparsity \cite{Cichocki04}.
\end{itemize}
...
Key & Description\\
\hline
\code{brunet} & Standard NMF. Based on Kullbach-Leibler divergence, it uses simple multiplicative updates from \cite{Lee2000}, enhanced to avoid numerical underflow.
...
%
\code{nsNMF} & Non-smooth NMF. Uses a modified version of Lee and Seung's multiplicative updates for Kullbach-Leibler divergence to fit a extension of the standard NMF model. 
...
$H_0$\footnote{Some algorithms only need one matrix factor (either $W$ or $H$) to be initialized. See for example the SNMF/R(L) algorithm of Kim and Park \cite{Kim2007}.}), from which to start the iteration process. 
...
It can directly perform NMF on object of class \code{matrix} or \code{data.frame} and \code{ExpressionSet} -- if the \code{Biobase} package is installed. 
...
\footnote{\code{ExpressionSet} is the base class for handling microarray data in BioConductor, and is defined in the \code{Biobase} package.}
...
The default algorithm is given by the package specific option \code{'default.algorithm'}, which defaults to \code{'brunet'} on installation \cite{Brunet04}.
...
It was used in several papers on NMF \cite{Brunet04, Gao2005} and is included in the \nmfpack package's data, wrapped into an \code{ExpressionSet} object.
...
Kim and Park \cite{Kim2007} defined a procedure to extract the relevant genes for each metagene, based on a gene scoring schema.
...
For example, to use the NMF algorithm from Lee and Seung \cite{Lee2000} based on the Frobenius euclidean norm, one make the following call: 
<<algo_lee, cache=TRUE>>=
...
In the case of multiple runs, the computations use RNGstream, as provided by the package \code{rstream} \cite{Lecuyer2002, Lecuyer2005}, which generates multiple independent random streams (one per run).
...
Moreover, single runs can also use RNGstream by readily passing a 6-length seed.
...
\subsubsection{Parallel computing on multi-core machines (non-Windows)}\label{multicore}
...
It uses the \code{foreach} framework developed by REvolution Computing \cite{foreach}, together with the related \code{doMC} parallel backend \cite{doMC} -- based on the \code{multicore} package -- to make use of all the CPUs available on the system.
Each core will simultaneously perform part of the runs. 
Therefore, the required memory increases linearly with the number of cores used.
When only the best run is of interest, the memory usage is optimized by using shared memory and mutex objects from the packages \code{bigmemory} and \code{synchronicity}, to only keep the current best factorization.

\medskip
\paragraph{IMPORTANT NOTE:} because it uses the \code{multicore} package, parallel computation over multi-cores is available only for Unix and Mac machines.
The parallel computation is based on the \code{doMC} and \code{multicore} packages, so the same care should be taken as stated in the vignette of \code{doMC}:
\begin{quote}
...
\bigskip
The default parallel backend used by the \code{nmf} function is defined by the package specific option \code{'parallel.backend'}, which defaults to \code{'mc'} 
-- for \code{doMC}.
The backend can also be set on runtime via argument \code{'.pbackend'}.

There are two other runtime options, \code{parallel} and \code{parallel.required}, that can be passed via argument \code{.options}, to control the behaviour of the parallel computation (see below).
...
In our tests we used the \code{doMPI} package to perform 100 factorizations using hybrid parallel computation on 4 quadri-core machines -- making use of all the cores computation on each machine.

The scripts used to launch and run the factorizations can be found in file \emph{mpi.R} in the package's \emph{examples} directory:
...
The script file \emph{mpi.R} contains some extra code to log and trace the computation. 
Reducing it to the essential gives the following piece of code:
...
This can be done via the option \code{'-p'} or \code{'p1'} by setting the parallel backend to \code{NULL}, \code{'seq'} or \code{{'}{'}}:
...
For example, \cite{Brunet04} proposed to take the first value of $r$ for which the cophenetic coefficient starts decreasing, \cite{Hutchins2008} suggested to choose the first value where the RSS curve presents an inflection point, and \cite{Frigyesi2008} considered the smallest value at which the decrease in the RSS is lower than the decrease of the RSS obtained from random data.
...
Whereas the standard NMF procedure usually involves several hundreds of random initialization, performing 30-50 runs is considered sufficient to get a robust estimate of the factorization rank \cite{Brunet04, Hutchins2008}.
...
<<estimate_rank_plot, fig=TRUE, width=10, height=6>>=
plot(estim.r)
@
...
<<estimate_rank_hm_include, fig=TRUE, width=14, height=7>>=
...
<<estimate_rank_random, include=FALSE, cache=TRUE>>=
...
@
<<estimate_rank_random_include, fig=TRUE, width=10, height=6>>=
...
The trajectory can be plot with the method \code{plot} (\cref{fig:error}):
<<errorplot_plot, include=FALSE>>=
plot(res)
@
...
The methods \code{basismap}, \code{coefmap} and \code{consensusmap} provide an easy way to visualize respectively the resulting basis matrix (i.e. metagenes), mixture coefficient matrix (i.e. metaprofiles) and the consensus matrix, in the case of multiple runs. 
It produces pre-configured heatmaps based on the function \code{aheatmap}. 
The default heatmaps produced by these functions are shown in \cref{fig:heatmap_coef_basis,fig:heatmap_consensus}.
They can be customized in many different ways (colours, annotations, labels).
See the dedicated vignette \emph{``NMF: generating heatmaps"} or the help pages \code{?coefmap} and \code{?aheatmap} for more information.
...
In \cref{fig:heatmap_coef_basis} we simply used \code{subsetRow=TRUE}, which subsets the rows using the method described in \cite{Kim2007}, to only keep the basis-specific features (e.g. the metagene-specific genes). 
...
<<heatmap_coef_basis, include=FALSE>>=
layout(cbind(1,2))
# basis components
...
\begin{figure}[ht]
\centering
<<heatmap_coef_basis_inc, fig=true, echo=FALSE, width=14, height=7>>=
<<heatmap_coef_basis>>
@
\caption{Heatmap of the basis and the mixture coefficient matrices. The rows of the basis matrix were selected using the default feature selection method -- described in \cite{Kim2007}.}
\label{fig:heatmap_coef_basis}
\end{figure}

\nbnote{Note how the function \texttt{aheatmap}, the underlying heatmap engine provided with the package NMF, makes it is possible to combine several heatmaps on the same plot, using the standard layout call \texttt{par(mfrow=...)}.
It is also directly compatible with both \texttt{layout(...)} and viewports from \texttt{grid} graphics.
The plotting context is autaomatically detected behind the scene, and a correct behaviour is achieved thanks to the package \texttt{gridBase}.}
...
\caption{Heatmap of consensus matrices from 10 runs on the limited dataset (left) and from 100 runs on the complete Golub dataset (right).}
...
\begin{multicols}{2}
Because the computation was performed with error tracking enabled, an error plot can be produced by method \code{plot} (\cref{fig:multi_error}).
...
<<multiple_errorplot, include=FALSE>>=
...
\setkeys{Gin}{width=0.5\textwidth}
\centering
<<multiple_errorplot, fig=TRUE, echo=FALSE>>=
<<multiple_errorplot>>
@
...
However, some NMF algorithms have different underlying models, such as non-smooth NMF \cite{nsNMF2006} which uses an extra matrix factor that introduces an extra parameter, and change the way the target matrix is approximated.
