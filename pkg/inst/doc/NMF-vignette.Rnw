...
multiplicative updates from \cite{Lee2000}, enhanced 
...
%
\code{nsNMF} & Nonsmooth NMF. Uses a modified version of Lee and Seung's 
...
The \nmfpack package is designed so that such algorithms -- that handle negative data -- can be 
integrated into its framework.  

By default, the sign of the input data is checked before running any method, so that the 
...
and plug mixed sign NMF methods into the framework, one needs to specify the argument 
...
available models can be retireved calling the \code{nmfModel()} function with no 
argument:
