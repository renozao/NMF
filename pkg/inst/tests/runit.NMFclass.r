...
test.subset <- function(){
	...
	checkEquals(dim(a[NULL]), c(n, 0), "subset basis NULL is OK (dim)")
	checkTrue(is.vector(a[2]), "subset with single index returns a vector if length = 1")
	checkTrue(is.vector(a[2, drop=TRUE]), "subset with single index + dropping returns a vector if length = 1")
	checkTrue(is.matrix(a[2, drop=FALSE]), "subset with single index + NOT dropping returns a matrix if length = 1")
	checkEquals(a[2], basis(a)[,2, drop=TRUE], "subset with single index returns correct vector if length = 1")
	checkEquals(a[2, drop=TRUE], basis(a)[,2, drop=TRUE], "subset with single index + dropping returns correct vector if length = 1")	
	checkEquals(a[2, drop=FALSE], basis(a)[,2, drop=FALSE], "subset with single index + NOT dropping returns correct matrix if length = 1")
	checkTrue(is.matrix(a[1:2]), "subset with single index returns a matrix if length > 1")
	checkTrue(is.matrix(a[1:2, drop=TRUE]), "subset with single index + dropping returns a matrix if length > 1")
	checkTrue(is.matrix(a[1:2, drop=FALSE]), "subset with single index + dropping returns a matrix if length > 1")
	checkEquals(a[2:3], basis(a)[,2:3], "subset with single index returns correct matrix if length > 1")
	checkEquals(a[2:3, drop=TRUE], basis(a)[,2:3, drop=TRUE], "subset with single index + dropping returns correct matrix if length > 1")
	checkEquals(a[2:3, drop=FALSE], basis(a)[,2:3, drop=FALSE], "subset with single index + NOT dropping returns correct matrix if length > 1")
