...
#' Measuring Distance between Target Matrix and NMF Model
#' 
#' The function \code{deviance} returns the approximation error between an NMF
#' model and its target matrix, as the value of the minimized objective
#' function.
#' 
#' The value is computed at the end of the fitting process by the function
#' \code{\link{nmf}}.
#' 
#' @name deviance
#' @aliases deviance,NMFfit-method deviance,NMFfitXn-method
#' @docType methods
#' @param object an \code{\linkS4class{NMFfit}} or
#' \code{\linkS4class{NMFfitXn}} object as returned by the function
#' \code{link{nmf}}.
#' @return a numeric value giving the value of the objective function for the
#' (best) fitted NMF model.
#' @seealso \code{\link{nmf}}, \code{\linkS4class{NMFfit}}
#' @keywords methods
#' @examples
#' 
#' 
...
#' NMF results.
#' 
#' The underlying function used to draw the heatmap is a mixture of the
#' function \code{heatmap.2} from the \code{gplots} package, and the function
#' \code{heatmap.plus} from the \code{heatmap.plus} package.  In particular, it
#' allows to add extra annotation rows and column using the arguments
#' \code{ColSideColor} and \code{RowSideColor} respectively.  See
#' \code{\link[gplots]{heatmap.2}} and
#' \code{\link[heatmap.plus]{heatmap.plus}}.
#' 
...
#' coefmap,NMFfitX-method
#' @docType methods
#' @param object An object that inherits from class \code{\linkS4class{NMF}} or
#' class \code{\linkS4class{NMFfitX}}.
...
#' annotates the consensus clusters obtained by hierachical clustering of the
#' consensus matrix with average linkage.  }
...
#' @param filter Argument that specifies how to filter the features that will
#' appear in the heatmap.  When \code{FALSE} (default), all the features are
#' used.
#' 
...
#' \code{\link{extractFeatures}}).
#' 
#' \item a \code{logical} vector of length \code{nrow(object)} (i.e. the number
#' of features) that specifies which features that should appear in the
#' heatmap.
#' 
#' \item a \code{numeric} vector of length greater than 1 that specifies the
#' index of the features that should appear in the heatmap.
#' 
...
#' metaHeatmap,matrix-method metaHeatmap,NMF-method metaHeatmap,NMF-method
...
#' nmfModel,matrix,matrix-method nmfModels
#' @docType methods
...
#' dispersion,matrix-method dispersion,NMFfitX-method consensusmap
#' consensusmap-methods consensusmap,NMFfitX-method consensusmap,matrix-method
#' consensusmap,NMF.rank-method consensusmap,list-method plot.NMF.consensus
#' nrun nrun-methods nrun,matrix-method predict,NMFfitXn-method runtime.all
...
#' }
#' 
#' \item{has.track}{Tells if an \code{NMFfit} object has a recorded error
#' track.  It should return \code{TRUE} if \code{object} was computed with the
#' option \code{track=TRUE} or the flag \code{'t'}.}
#' 
...
#' featureScore,NMF-method featureScore,matrix-method has.track nmfApply
...
#' getRNG,rstream-method getRNG1 getRNG1-methods getRNG1,NMFfit-method
...
#' @param packed a logical that specifies if the returned \code{rstream} object
...
#' the extracted object (see details). For \code{NMFfit} objects, the result is
#' an \code{\linkS4class{rstream}} object that contains the RNG settings as
...
#' initialize,rstream.user-method RNGinfo RNGhooks RNGlib RNGlibs RNGprovider
#' RNGrecovery RNGseed RNGseq RNGwrap setRNG setRNG-methods setRNG,ANY-method
#' setRNG,character-method setRNG,numeric-method setRNG,rstream-method
#' @author Renaud Gaujoux
...
#' @param what Specifies on which matrix, basis components
#' (\code{what='features'}) or mixture coefficients (\code{what='samples'}) the
