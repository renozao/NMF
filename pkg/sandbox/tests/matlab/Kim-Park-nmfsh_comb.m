...
%
%
function [W,H,i,obj]=nmfsh_comb(A,k,param, W,verbose,bi_conv,eps_conv)

if nargin<7, eps_conv=1e-4;, end
if nargin<6, bi_conv=[0 10];, end
...
% compute objevtive function
if( mod(i,5)==0 )
obj_cur = 1/2 * ( sum(sum( (A - (W * H)).^2 )) + eta * sum(sum(W.^2)) + beta * sum( sum( H, 1 ).^2 ) );
obj = [obj obj_cur];
end
