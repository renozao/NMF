...
\bibliography{/tmp/Rpkglib_45c34672d7c3/NMF/REFERENCES}
...
\begin{tabular}{rrrr}
 ...
 & C & R & Speed.up \\ 
  \hline
brunet & 2.40 & 8.42 & 3.51 \\ 
  lee & 5.66 & 9.97 & 1.76 \\ 
  offset & 5.29 & 14.02 & 2.65 \\ 
  nsNMF & 4.72 & 12.33 & 2.61 \\ 
   \hline
\end{tabular}
...
## brunet    543536   510 0.264   0.264    1
## lee    673513121  1780 1.108   1.108    1
## nsNMF     585106   970 0.728   0.728    1
\end{verbatim}
\begin{alltt}
...
## brunet           0.7488    543536   510 0.264   0.264    1
## lee              0.6764 673513121  1780 1.108   1.108    1
## nsNMF            0.7946    585106   970 0.728   0.728    1
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\hlstd{my.algorithm} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{,} \hlkwc{seed}\hlstd{,} \hlkwc{scale.factor} \hlstd{=} \hlnum{1}\hlstd{) \{}
    ...
    \hlkwd{metagenes}\hlstd{(}\hlkwd{fit}\hlstd{(seed))} \hlkwb{<-} \hlkwd{abs}\hlstd{(pca}\hlopt{$}\hlstd{rotation[,} \hlnum{1}\hlopt{:}\hlstd{factorization.rank])}
    ...
    \hlcom{# parameters)}
    \hlkwd{metaprofiles}\hlstd{(}\hlkwd{fit}\hlstd{(seed))} \hlkwb{<-} \hlkwd{t}\hlstd{(}\hlkwd{abs}\hlstd{(pca}\hlopt{$}\hlstd{x[,} \hlnum{1}\hlopt{:}\hlstd{factorization.rank]))}\hlopt{/}\hlstd{scale.factor}
...
\hlstd{V} \hlkwb{<-} \hlkwd{syntheticNMF}\hlstd{(n, r, p,} \hlkwc{noise} \hlstd{=} \hlnum{TRUE}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlkwd{nmf}\hlstd{(V,} \hlnum{3}\hlstd{, my.algorithm,} \hlkwc{scale.factor} \hlstd{=} \hlnum{10}\hlstd{,} \hlkwc{objective} \hlstd{=} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{,}
    \hlkwc{target}\hlstd{,} \hlkwc{...}\hlstd{) \{}
    \hlstd{(}\hlkwd{sum}\hlstd{((target} \hlopt{-} \hlkwd{fitted}\hlstd{(x))}\hlopt{^}\hlnum{4}\hlstd{))}\hlopt{^}\hlstd{\{}
...
\hlstd{my.algorithm.offset} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{,} \hlkwc{seed}\hlstd{,} \hlkwc{scale.factor} \hlstd{=} \hlnum{1}\hlstd{) \{}
    ...
    \hlkwd{metagenes}\hlstd{(data.model)} \hlkwb{<-} \hlkwd{abs}\hlstd{(pca}\hlopt{$}\hlstd{rotation[,} \hlnum{1}\hlopt{:}\hlstd{factorization.rank])}
    \hlcom{# use the rotated matrix to get the mixture coefficient use a}
    \hlcom{# scaling factor (just to illustrate the use of extra}
    \hlcom{# parameters)}
    \hlkwd{metaprofiles}\hlstd{(data.model)} \hlkwb{<-} \hlkwd{t}\hlstd{(}\hlkwd{abs}\hlstd{(pca}\hlopt{$}\hlstd{x[,} \hlnum{1}\hlopt{:}\hlstd{factorization.rank]))}\hlopt{/}\hlstd{scale.factor}
...
\hlstd{my.seeding.method} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{model}\hlstd{,} \hlkwc{target}\hlstd{) \{}
    ...
    \hlkwd{metagenes}\hlstd{(model)} \hlkwb{<-} \hlstd{target[,} \hlkwd{order}\hlstd{(w.cols)[}\hlnum{1}\hlopt{:}\hlkwd{nbasis}\hlstd{(model)]]}
    \hlcom{# initialize H randomly}
    \hlkwd{metaprofiles}\hlstd{(model)} \hlkwb{<-} \hlkwd{matrix}\hlstd{(}\hlkwd{runif}\hlstd{(}\hlkwd{nbasis}\hlstd{(model)} \hlopt{*} \hlkwd{ncol}\hlstd{(target)),}
...
\begin{itemize}\raggedright
  ...
  \item Other packages: BH~1.51.0-2, bigmemory~4.4.3,
    bigmemory.sri~0.1.2, Biobase~2.20.1,
    BiocGenerics~0.6.0, cluster~1.14.4,
    doParallel~1.0.3, foreach~1.4.1, iterators~1.0.6,
    knitr~1.4.9, NMF~0.17.9, pkgmaker~0.17,
    RColorBrewer~1.0-5, registry~0.2, rngtools~1.2.2,
    synchronicity~1.1.0, xtable~1.7-1
  \item Loaded via a namespace (and not attached):
    codetools~0.2-8, colorspace~1.2-2,
    compiler~3.0.1, dichromat~2.0-0, digest~0.6.3,
    evaluate~0.4.7, formatR~0.9, ggplot2~0.9.3.1,
    grid~3.0.1, gridBase~0.4-6, gtable~0.1.2,
    highr~0.2.1, labeling~0.2, MASS~7.3-29,
    munsell~0.4.2, plyr~1.8, proto~0.3-10,
    reshape2~1.2.2, scales~0.2.3, stringr~0.6.2,
    tools~3.0.1
\end{itemize}
