...
\hlcomment{# add an annotation track for the true groups}
Groups <- X$gcoef
\end{alltt}


{\ttfamily\noindent\bfseries\textcolor{errorcolor}{\#\# Error: Could not access attribute via `\$`: attribute 'gcoef' is not exposed. Use `attr(x, 'gcoef').}}\begin{alltt}
c <- \hlfunctioncall{rnorm}(p)
\hlcomment{# gather them in a data.frame}
covariates <- \hlfunctioncall{data.frame}(a, Groups, c)
\end{alltt}


{\ttfamily\noindent\bfseries\textcolor{errorcolor}{\#\# Error: object 'Groups' not found}}\end{kframe}
\end{knitrout}
...
specifying e.g. \code{annCol=c(Metagene=':basis', 'consensus')}.
...
\hlfunctioncall{coefmap}(res, annCol=NA)
...
\hlcomment{# customized plot}
\hlfunctioncall{basismap}(res, main = \hlstring{"Metagenes"}, annRow = \hlfunctioncall{list}(\hlstring{":basis"}, d, e))
\hlfunctioncall{par}(opar)
\end{alltt}
...
{\ttfamily\noindent\bfseries\textcolor{errorcolor}{\#\# Error: object 'covariates' not found}}\begin{alltt}
\hlfunctioncall{par}(opar)
\end{alltt}
...
\begin{itemize}\raggedright
  ...
  \item Other packages: bigmemory~4.3.0, bigmemory.sri~0.1.2,
    ...
    NMF~0.8.24, pkgmaker~0.8.17, RColorBrewer~1.0-5, registry~0.2,
