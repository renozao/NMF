...
\Sexpr{utils::packageVersion('NMF')}}
...
res1 <- nmf(esGolub, 3, nrun=5, .opt='v-p', seed=123)

# or use the SEQ backend of foreach: .pbackend=NA or 'seq'
res2 <- nmf(esGolub, 3, nrun=5, .opt='vp1', seed=123)
res3 <- nmf(esGolub, 3, nrun=5, .opt='v', .pbackend='seq', seed=123)
res4 <- nmf(esGolub, 3, nrun=5, .opt='v', .pbackend=NA, seed=123)

# all results are all identical
nmf.equal(list(res1, res2, res3, res4))
@
