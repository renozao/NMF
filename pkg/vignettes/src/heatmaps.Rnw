...
\Rcode{annRow=NA}, displayed separately by passing only one of the given names 
(e.g. \Rcode{annRow='basis'}), and their legend names may be changed by
specifying e.g.
\Rcode{annRow=c(Metagene='basis', 'consensus')}.
...
Again, the track can be disabled by setting \Rcode{annRow=NA}, and extra 
