...
a modification of the function \code{pheatmap} from the package \code{pheatmap} 
...
the same argument \code{annCol} (see \ref{sec:aheatmap} or \code{?aheatmap} for
more details).
...
basismap(res)
# customized plot
basismap(res, main="Metagenes", annRow=list('basis', Var=d, Category=e))
par(opar)
@
...
In section \ref{sec:data}, the NMF fit \code{res} was computed with argument 
...
<<consensusmap_methods, fig.width=10, fig.height=7>>=
