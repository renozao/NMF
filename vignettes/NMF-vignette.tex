...
\bibliography{/tmp/Rpkglib_394351b45565/NMF/REFERENCES}
...
##        7.348e-01        5.435e+05        5.100e+02        2.450e-01 
##          cpu.all             nrun 
##        2.450e-01        1.000e+00
\end{verbatim}
\begin{alltt}
...
##        5.100e+02        2.450e-01        2.450e-01        1.000e+00
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# the default call will try to run in parallel using all the cores => will}
\hlcom{# be in parallel if all the requirements are satisfied}
...
## brunet          0.8126           0.7488    543536   510 0.260   0.260    1
## lee             0.7148           0.6764 673513121  1780 1.105   1.105    1
## nsNMF           0.8654           0.7946    585106   970 0.740   0.740    1
\end{verbatim}
\begin{alltt}
...
\hlstd{my.algorithm} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{,} \hlkwc{seed}\hlstd{,} \hlkwc{scale.factor} \hlstd{=} \hlnum{1}\hlstd{) \{}
    \hlcom{# do something with starting point ...  for example: 1. compute principal}
    \hlcom{# components}
    \hlstd{pca} \hlkwb{<-} \hlkwd{prcomp}\hlstd{(}\hlkwd{t}\hlstd{(x),} \hlkwc{retx} \hlstd{=} \hlnum{TRUE}\hlstd{)}

    \hlcom{# 2. use the absolute values of the first PCs for the metagenes Note: the}
    \hlcom{# factorization rank is stored in object 'start'}
    ...
    \hlcom{# use the rotated matrix to get the mixture coefficient use a scaling factor}
    \hlcom{# (just to illustrate the use of extra parameters)}
...
\begin{itemize}\raggedright
  ...
  \item Other packages: BH~1.51.0-4, bigmemory~4.4.6,
    ...
    cluster~1.14.4, knitr~1.5, NMF~0.20.2, pkgmaker~0.20,
  ...
  \item Loaded via a namespace (and not attached):
    ...
    formatR~0.10, ggplot2~0.9.3.1, grid~3.1.0, gridBase~0.4-7,
